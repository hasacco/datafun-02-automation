...
pet1: str = input("Enter a type of pet: ")
pet2: str = input("Enter another type of pet: ")
pet3: str = input("Enter one more type of pet: ")
pet_list: list[str] = [pet1, pet2, pet3]
...
def create_files_from_numeric_range() -> None:
    ...
    for quarter_number in range(FIRST_QUARTER, LAST_QUARTER + 1):
        filename: str = f"hasacco_quarter_{quarter_number}.txt"
...
def create_files_from_list() -> None:
    ...
    for pet_name in pet_list:
        filename: str = f"{user_name}_{pet_name}.txt"
        path: Path = PROCESSED_DIR / filename
...
def create_files_using_list_comprehension() -> None:
    ...
    for favorite in favorite_list:
        filename: str = f"{user_name}_{favorite}.txt"
        path: Path = PROCESSED_DIR / filename
...
def create_files_periodically() -> None:
    ...
    while i <= FILE_COUNT:
        filename: str = f"hasacco_{i:02d}.txt"
        path: Path = PROCESSED_DIR / filename
